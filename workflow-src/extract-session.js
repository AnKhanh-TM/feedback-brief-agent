...
const source = $('BH | Chuẩn bị prompt tiếng Việt').first().json;
return [{ json: { reportType: 'session', period: source.period, metrics: source.metrics, alertCount: source.alertCount, reportMarkdown: text, openaiResponseId: response.id, generatedAt: new Date().toISOString() } }];
